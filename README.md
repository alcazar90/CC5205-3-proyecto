...
Esta última tabla contiene valores que describen características de las canciones que nos serán útiles para clasificar o predecir preferencias. Podemos notar que energy, tempo, danceability y valence (A) tienen una distribución simétrica y equitativamente repartida en el rango. Acousticness, speechness, liveness e instrumentalness (B) tienen distribuciones muy cargadas hacia valores bajos. Pareciesen comportarse como características binarias, donde si no posee la característica tendrá valor 0, y si la posee tendrá algún valor >0 uniformemente distribuido. Sería conveniente transformarlas matemáticamente o pasarlas a variables categóricas. Duration y loudness (C) presentan outliers que ensucian los datos, por lo que sería necesario eliminarlos. Finalmente, mode, time_signature y key (D) toman una cantidad limitada de valores. Mode al tomar sólo 2 valores debería ser considerado como variable categórica. Notamos que en time signature hay outliers que posiblemente son datos erroneos (No existen compás 0/4). También sería necesario transformarla, ya que no hace sentido considerarla una variable ordinal.
